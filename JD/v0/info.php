...
<TABLE BORDER="1">
  ...
  <form method="POST" action="/supression_message.php">
 <?php
try
{

    $pdo_options[PDO::ATTR_ERRMODE] = PDO::ERRMODE_EXCEPTION;
    $bdd = new PDO('mysql:host=127.0.0.1;dbname=expernet','root','',$pdo_options);
     
    


    // On recupere tout le contenu de la table news
$reponse = $bdd->query('SELECT id, allergenes, typeRepas, prix, taxe, temps FROM plat');
...
while ($donnees = $reponse->fetch())
{
    ...
    echo "<td> $donnees[allergenes] </td>";
    echo "<td> $donnees[typeRepas] </td>";
    echo "<td> $donnees[taxe] </td>";
    echo "<td> $donnees[temps] </td>";
    
    echo "<td>
    <button style='background-color:#398AA4;color:white;' type='submit' name='suppr' value='".$donnees['id']."'>Supprimer</button>
    </td>";
    echo "</tr>";
